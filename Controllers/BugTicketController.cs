...
namespace BugTracker.Controllers
{
    public class BugTicketController : Controller
    {
        ...
        public ActionResult Details(int ticketId)
        {
            ...
            // Setup the queries and the parameter.
            ...
            Ticket currTicket = _db.Query<Ticket>(getTicketQuery, parameterTicketId).First();
            string projectTitle = _db.Query<string>(getProjectTitleQuery, parameterTicketId).First();
            string userOpenedName = _db.Query<string>(getTicketOpenedByName, parameterTicketId).First();

            // Get the comments for this ticket.
            string getCommentsFromTicket = "SELECT * FROM Comments WHERE Comments.TicketId = @queryTicketId;";
            List<Comment> commentList = _db.Query<Comment>(getCommentsFromTicket, parameterTicketId).ToList();
            ...
            BugTicketDetailsViewModel model = new BugTicketDetailsViewModel()
            {
                ...
                CommentList = commentController.CommentListToReadable( commentList )
            };
        ...
        public ActionResult Create(IFormCollection collection)
        {
            ...
            // Generate a TicketId
            string getMaxTicketIdQuery = "SELECT Tickets.Id FROM Tickets ORDER BY Tickets.Id DESC;";
            // Make sure there is something in the database. Default is 0.
            extractedMax = _db.Query<int>(getMaxTicketIdQuery).FirstOrDefault();
            int ticketId = extractedMax + 1;

            // Add Ticket to myDB.
            string insertTicketQuery = "INSERT INTO [Tickets] VALUES( @projParent,  @queryTicketId,  @historyId, @isCurr, @title, @severity, @status, @unwantedBehavior, @repeatableSteps, @openedBy, @dateCreated );";
            var parameters = new
            {
                projParent = projId,
            ...
            _db.Execute(insertTicketQuery, parameters);
